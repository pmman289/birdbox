...
export async function checkIncludeNodeAccess(nodeInput: unknown): Promise<NodeCommandResult> {
  ...
  const command = [
    ...
    // generated.conf is controller-owned output, but older installations may
    // have it as a regular file after manual edits; read access is sufficient
    // because stage/apply will replace it atomically with the new candidate.
    `[ ! -e '${node.generatedConfigPath}' ] || test -r '${node.generatedConfigPath}' || { echo 'SSH 用户无法读取 Birdbox 生成配置：${node.generatedConfigPath}' >&2; exit 1; }`,
...
export async function stageAndValidate(nodeInput: unknown, bundleInput: string | NodeConfigBundle): Promise<NodeCommandResult> {
  ...
  if (node.deploymentMode === "include") {
    ...
    const deviceCheck = await executeNodeCommand(node, [
      ...
      activeDeviceCheckCommand(node.mainConfigPath),
      `[ ! -e '${node.generatedConfigPath}' ] || test -r '${node.generatedConfigPath}'`,
    ].join("\n"), { timeout: 12_000 });
...
export async function applyStagedConfig(nodeInput: unknown, bundleInput: string | NodeConfigBundle = ""): Promise<NodeCommandResult> {
  ...
  if (node.deploymentMode === "include") {
    ...
    const command = [
      ...
      `current_kind=missing; current_target=''; current_file=''; current_backup='${activePath}.apply-backup.$$'; if [ -L '${activePath}' ]; then current_kind=symlink; current_target=$(readlink '${activePath}'); current_file=$(readlink -f '${activePath}'); elif [ -f '${activePath}' ]; then current_kind=file; current_file='${activePath}'; cp -p '${activePath}' "$current_backup"; fi`,
      ...
      `rm -f '${rollbackLink}'; case "$current_kind" in symlink) ln -sfn "$current_target" '${rollbackLink}';; file) cp -p "$current_backup" '${rollbackLink}';; esac`,
      ...
      `if birdc -s '${node.socketPath}' 'configure check' && birdc -s '${node.socketPath}' configure; then cleanup_versions || true; rm -f "$current_backup"; exit 0; fi`,
      ...resourceRollbackCommands(bundle, directory),
      `rm -f '${activePath}'; case "$current_kind" in symlink) ln -sfn "$current_target" '${switchLink}'; mv -f '${switchLink}' '${activePath}';; file) cp -p "$current_backup" '${activePath}';; esac; rm -f "$current_backup"`,
...
export async function rollbackNode(nodeInput: unknown, bundleInput: string | NodeConfigBundle = ""): Promise<NodeCommandResult> {
  ...
  if (node.deploymentMode === "include") {
    ...
    const command = [
      "set -eu",
      `current_kind=missing; current_target=''; current_backup='${activePath}.rollback-backup.$$'; if [ -L '${activePath}' ]; then current_kind=symlink; current_target=$(readlink '${activePath}'); elif [ -f '${activePath}' ]; then current_kind=file; cp -p '${activePath}' "$current_backup"; fi`,
      `rollback_kind=missing; rollback_target=''; if [ -L '${rollbackLink}' ]; then rollback_kind=symlink; rollback_target=$(readlink '${rollbackLink}'); elif [ -f '${rollbackLink}' ]; then rollback_kind=file; fi`,
      `test "$rollback_kind" != missing`,
      ...resourceRollbackCommands(bundle, path.posix.dirname(activePath)),
      `rm -f '${activePath}'; if [ "$rollback_kind" = symlink ]; then ln -sfn "$rollback_target" '${switchLink}'; mv -f '${switchLink}' '${activePath}'; else cp -p '${rollbackLink}' '${activePath}'; fi`,
      `if birdc -s '${node.socketPath}' 'configure check' && birdc -s '${node.socketPath}' configure; then rm -f '${rollbackLink}'; case "$current_kind" in symlink) ln -sfn "$current_target" '${rollbackLink}';; file) cp -p "$current_backup" '${rollbackLink}';; esac; rm -f "$current_backup"; exit 0; fi`,
      `rm -f '${activePath}'; case "$current_kind" in symlink) ln -sfn "$current_target" '${switchLink}'; mv -f '${switchLink}' '${activePath}';; file) cp -p "$current_backup" '${activePath}';; esac; rm -f "$current_backup"`,
      `birdc -s '${node.socketPath}' configure >/dev/null 2>&1 || true`,
      "exit 1",
    ].join("\n");
